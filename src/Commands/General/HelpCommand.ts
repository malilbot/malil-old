...
export default class HelpCommand extends Command {
	...
	public exec(message: Message, { command }: { command: Command }) {
		...
		const embed = new MessageEmbed()
			...
			.setDescription(
				'For help and bugs visit the [support server](https://discord.gg/mY8zTARu4g).'
			)
			.setFooter(
				`@malil help [ command ] for more information on a command. for help visit the`
			);
