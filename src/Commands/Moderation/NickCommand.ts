...
export default class NickCommand extends Command {
    ...
    public async exec(message, { user, name }) {
        ...
        if(message.mentions.members.first().roles.highest.position > message.guild.members.resolve(this.client.user).roles.highest.position)
    return message.channel.send("Sorry i cant change the nickname of that use cause his highest role is higher than mine");
