...
export default class CreatetagCommand extends Command {
    ...
    public async exec(message: Message, { args }) {
        if (!message.member.hasPermission('ADMINISTRATOR') || message.member.hasPermission('MANAGE_MESSAGES')) return message.channel.send("Sorry you dont hae the required permissions to use this command")
        ...
        .addFields(
            {name: args[0], value: input}
        )
        message.reply(embed)
        let pp = this.client.tags.get(message.guild.id)
        console.log(pp)
    }
}
