...
rl.question(green("What is the commit message?:  "), function(answer) {
            ...
            exec(`git add --all`, async (error, stdout, stderr) => {await console.log(error|| stdout|| stderr)})
            exec(`git commit -m "${answer}"`, async (error, stdout, stderr) => {await console.log(error|| stdout|| stderr)})
            rl.question(green("DO you want to push?:  "), function(answer) {
              if(answer == 'yes' || answer == 'y'){
                exec(`git push`, async (error, stdout, stderr) => {await console.log(error || stdout || stderr)})
